...
class RideController extends Controller
{
    ...
    public function findFareAndDistance(Request $request)
    {
        ...
        foreach ($services as $service) {
            ...
            $serviceData['min_amount']       = getAmount($service->$minColumName * $distance);
            $serviceData['max_amount']       = getAmount($service->$maxColumName * $distance);
            $serviceData['recommend_amount'] = getAmount($service->$recColumName * $distance);
            $data[]                          = $serviceData;
        }
    ...
    public function create(Request $request)
    {
        ...
        if ($pickUpZone->id == $destinationZone->id) {  // city ride
            $data['min_amount']            = getAmount(getRideMiniumAmount($service->city_min_fare) * $distance);
            ...
            $data['min_amount']            = getAmount(getRideMiniumAmount($service->intercity_min_fare) * $distance);
        ...
        if ($distance < gs('min_distance')) {
            $notify[] = 'Minimum distance must be ' . getAmount(gs('min_distance')) . ' ' . gs('distance_unit');
    ...
    public function cancel(Request $request, $id)
    {
        ...
        $message = $penaltyApplied
            ? 'You’ve been charged ' . showAmount(gs('user_cancellation_penalty')) . ' for ride cancellation'
    ...
    public function accept($bidId)
    {
        ...
        if ($activeRide) {
            $notify[] = 'You have an active ride';
        ...
        if ($runningRide) {
            $notify[] = 'You have a running ride';
